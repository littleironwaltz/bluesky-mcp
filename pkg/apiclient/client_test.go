...
func TestGet(t *testing.T) {
	...
	// Verify the response with auth token
	if err := json.Unmarshal(response, &result); err != nil {
		t.Errorf("Failed to parse authenticated response: %v", err)
	}

	if success, ok := result["success"].(bool); !ok || !success {
		t.Errorf("Authenticated request: Expected success: true, got %v", result["success"])
	}
}
